...
impl DataBase {
    fn calculate_nimber(&mut self, game: &ClosedGeneralizedNimGame) -> u16 {
        ...
        let nimbers: &mut Vec<u16> = &mut unique_child_games
            .into_iter()
            .map(|g| self.get_nimber(&g))
            .collect();
    ...
    pub fn get_latex_repr(&self) -> String {
        let mut key_value_pairs: Vec<(ClosedGeneralizedNimGame, u16)> =
        ...
        for (g, nimber) in key_value_pairs {
            ...
            let latex_repr = match Pit::try_reconstruct(&g) {
...
fn k_v_comparer(
    (key1, val1): &(ClosedGeneralizedNimGame, u16),
    (key2, val2): &(ClosedGeneralizedNimGame, u16),
) -> Ordering {
